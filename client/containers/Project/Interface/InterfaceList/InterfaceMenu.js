...
class InterfaceMenu extends Component {
  ...
  handleAddInterfaceCat = data => {
    ...
    axios.post('/api/interface/add_cat', data).then(res => {
      ...
      message.success('the category of interface was added successfully');
      this.getList();
  ...
  handleChangeInterfaceCat = data => {
    ...
    axios.post('/api/interface/up_cat', params).then(res => {
      ...
      message.success('categories of interface were successfully updated ');
      this.getList();
      this.props.getProject(data.project_id);
      this.setState({
        change_cat_modal_visible: false
  ...
  showConfirm = data => {
    ...
    const ref = confirm({
      ...
      content: 'tips：interface can not recovered after deleting',
      okText: 'confirm',
      cancelText: 'cancel',
      async onOk() {
  ...
  showDelCatConfirm = catid => {
    let that = this;
    const ref = confirm({
      title: 'are you sure to delete this category of interface？',
      content: 'tips：this operation will delete all the interface under this category . They cannot be recovered after deleting.',
      okText: 'confirm',
      cancelText: 'cancel',
      async onOk() {
  ...
  copyInterface = async id => {
    ...
    axios.post('/api/interface/add', newData).then(async res => {
      ...
      message.success('interface successfully added');
      let interfaceId = res.data.data._id;
      await this.getList();
      this.props.history.push('/project/' + this.props.projectId + '/interface/api/' + interfaceId);
      this.setState({
  ...
  render() {
    ...
    const searchBox = (
      <div className="interface-filter">
        <Input onChange={this.onFilter} value={this.state.filter} placeholder="search interface" />
        <Button
          type="primary"
